...
window.onload = function(){
    ...
    var purl=conf.ur+"/api/admin/listToAuditTopics";
    $.ajax({
        url: purl,
        type: 'post',
        dataType: 'json',
        ...
        success: function (data) {
            if (data.code==0){
                ...
                for (var i=0;i<dt.length;i++){
                    ...
                    tabledetail+="<td>"+"<button type='button' class='btn btn-success' onclick='doAudit("+dt[i].id+")'>审核</button>"+"</td>>";
                    tabledetail+="</tr>"
                }
...
function doAudit(topicid) {
    var purl=conf.ur+"/api/admin/doAudit";
    if(!window.confirm('确定将此课题通过审核吗？')){
        return;
    }
    $.ajax({
        url: purl,
