...
window.onload = function(){
    ...
    var purl=conf.ur+"/api/student/listChoosedTopics";
    $.ajax({
        url: purl,
        type: 'post',
        dataType: 'json',
        ...
        success: function (data) {
            if (data.code==0){
                ...
                for (var i=0;i<dt.length;i++){
                    ...
                    // tabledetail+="<td>"+"<button type='button' class='btn btn-success' onclick='stuReturnTopicAction("+dt[i].id+")'>退选</button>"+"</td>>";
                    tabledetail+="</tr>"
                }
...
function stuReturnTopicAction(topicid) {
    if (userToken==null||userToken===""){
        alert("请先登录！");
        window.location.href=conf.ur+"/login";
        return;
    ...
    var purl=conf.ur+"/api/student/stuReturnTopicAction";
    if(!window.confirm('确定退选此课题吗？')){
        return;
    }else if(!window.confirm('请再次确认是否要退选该课题！')){
        return;
    }
    $.ajax({
        url: purl,
